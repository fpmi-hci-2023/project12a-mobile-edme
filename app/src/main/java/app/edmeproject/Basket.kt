...
@Composable
fun Basket(orderState: MutableState<OrderState>) {
    ...
    Column {
        ...
        if (basketList.isNotEmpty()) {
            ...
            Button(onClick = {
                orderState.value = OrderState.CHECKOUT_ORDER
            }, modifier = Modifier
                .padding(all = 8.dp)
                .fillMaxWidth()
            ) {
...
fun BasketPreview() {
    val orderState = rememberSaveable { mutableStateOf(OrderState.BASKET) }
    EdmeProjectTheme {
        Basket(orderState)
    }
}
...
@Composable
fun CheckoutOrder(orderState: MutableState<OrderState>) {
    ...
    Column {
        LazyColumn(modifier = Modifier.weight(1f)) {
            item {
                Row(modifier = Modifier.clickable {
                    orderState.value = OrderState.BASKET
                } ) {
        ...
        Button(onClick = {
            if (phoneNumber.isEmpty()) {
                ...
                orderState.value = OrderState.DETAILS
            }
        }, modifier = Modifier
            .padding(all = 8.dp)
            .fillMaxWidth()
...
fun CheckoutOrderPreview() {
    val orderState = rememberSaveable { mutableStateOf(OrderState.CHECKOUT_ORDER) }
    EdmeProjectTheme {
        CheckoutOrder(orderState)
    }
}

@Composable
fun DeliveryDetails(orderState: MutableState<OrderState>,
    ...
    val radioOptions = listOf(stringResource(id = R.string.pick_up_point),
        stringResource(id = R.string.home_work))
    ...
    Column {
        LazyColumn(modifier = Modifier.weight(1f)) {
            ...
            item {
                Text(
                    text = stringResource(R.string.work_details),
                    modifier = Modifier
                        .padding(all = 8.dp)
                        .fillMaxWidth(),
                    fontSize = 36.sp,
